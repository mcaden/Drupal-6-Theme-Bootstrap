 ...
 *   A form array.
 */
function contract_settings($saved_settings, $subtheme_defaults = array()) {
  /*
  ...
  drupal_add_css(drupal_get_path('theme', 'contract') . '/css/theme-settings.css', 'theme');

  // Add Javascript to adjust the layout on the settings page
  // drupal_add_js(drupal_get_path('theme', 'contract') . '/css/theme-settings.js', 'theme');

  // Get the default values from the .info file.
  $defaults = contract_theme_get_default_settings('contract');
  ...
  $form['contract_dev']['contract_block_edit_links'] = array(
     '#type'          => 'checkbox',
     '#title'         => t('Display block editing links.'),
     '#default_value' => $settings['contract_block_edit_links'],
     '#description'   => t('When hovering over blocks, display edit links for the proper users.'),
    );

  // Setting for flush all caches
  $form['contract_dev']['contract_rebuild_registry'] = array(
     '#type'          => 'checkbox',
     '#title'         => t('Rebuild theme registry on every page.'),
     '#default_value' => $settings['contract_rebuild_registry'],
